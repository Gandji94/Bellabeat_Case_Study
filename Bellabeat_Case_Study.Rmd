...
daily_intensities <- read_csv("/cloud/project/dailyIntensities_merged.csv")
hourly_calories <- read_csv("/cloud/project/hourlyCalories_merged.csv")
hourly_intensities <- read_csv("/cloud/project/hourlyIntensities_merged.csv")
hourly_steps <- read_csv("/cloud/project/hourlySteps_merged.csv")
minute_sleep <- read_csv("/cloud/project/minuteSleep_merged.csv")
...
```{r warning=FALSE, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
total_steps %>% 
  summarize(mean=mean(total_steps_da), min=min(total_steps_da), max=max(total_steps_da))
```
...
calories_plot_am <- ggplot(hours_am, aes(x=time_am, y=avg_am))+
  ...
  labs(title = "Avg. Calorie Consumption in the AM Period")+
...
calories_plot_pm <- ggplot(hours_pm, aes(x=time_pm, y=avg_pm))+
  geom_bar(stat = "identity")+
  labs(title = "Avg. Calorie Consumption PM")+
  theme(axis.text = element_text(angle = 45))
...
The most steps in the PM period are accumulated at 12 PM, 2 PM and 5 PM-7 PM.  
...
```{r warning=FALSE, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
ggplot(weekday_act_sleep_df, aes(x=weekday, y=wd_steps))+geom_bar(stat = "identity")+labs(title="Avg. Steps during a Week")
```  
...
```{r warning=FALSE, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
ggplot(weekday_act_sleep_df, aes(x=weekday, y=wd_sleep))+geom_bar(stat = "identity")+labs(title="Avg. Minutes Asleep during a Week")
```

Regarding sleep, the sleeping pattern did not change much over the period of a week (avg. 7h).  

```{r warning=FALSE, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
ggplot(weekday_act_sleep_df, aes(x=weekday, y=wd_calories))+geom_bar(stat = "identity")+labs(title="Avg. Calorie consumption during a Week")
```
